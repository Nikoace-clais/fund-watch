"""OCR fund-import pipeline: OCR → AI extract → verify → search → Pro review.

Pure business logic; the router only streams the SSE events produced here.
"""
...
def _share_class(name: str) -> str:
    ...
    pat = r"([ABCDEI])(?:人民币|美元|美钞|美汇|现汇|现钞)?基?金?$"
    m = _re.search(pat, name.strip())
...
async def ocr_fund_generator(
    ...
    for filename, data in files_data:
        ...
        try:
            ...
            yield _sse("error", text=ai_error(e))
            return
