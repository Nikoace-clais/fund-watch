"""Integration tests for OCR endpoints.

The OCR engine itself is mocked (no model download / inference); these cover
the route logic: threadpool wiring, SSE result streaming, code verification,
name-search fallback merge and the ocr_records insert.
"""

import io
import json

import app.db as app_db
import app.routers.ocr as ocr_router
import pytest
from app.main import app as fastapi_app

from tests.client import ASGISyncClient

# Minimal fund universe the fake search source knows about
_FUNDS = {
    "005827": {"code": "005827", "name": "易方达蓝筹精选混合", "type": "混合型"},
    "161725": {"code": "161725", "name": "招商中证白酒指数A", "type": "指数型"},
}


@pytest.fixture
def ocr_client(tmp_path, monkeypatch):
    """TestClient with temp DB and temp upload dir."""
    monkeypatch.setattr(app_db, "DB_PATH", tmp_path / "test.db")
    monkeypatch.setattr(ocr_router, "UPLOAD_DIR", tmp_path)

    async def direct_threadpool(func, *args, **kwargs):
        return func(*args, **kwargs)

    monkeypatch.setattr(ocr_router, "run_in_threadpool", direct_threadpool)
    app_db.init_db()
    return ASGISyncClient(fastapi_app)


@pytest.fixture
def fake_search(monkeypatch):
    """search_fund_by_name stub: looks up by exact code or name substring."""

    async def _search(q, limit=1):
        hits = [
            f
            for f in _FUNDS.values()
            if f["code"] == q or q in f["name"] or f["name"] in q
        ]
        return hits[:limit]

    monkeypatch.setattr(ocr_router, "search_fund_by_name", _search)
    return _search


def _png_upload():
    return {"file": ("shot.png", io.BytesIO(b"fake-png-bytes"), "image/png")}


def _fund_upload():
    return {"files": ("shot.png", io.BytesIO(b"fake-png-bytes"), "image/png")}


def _sse_result(resp):
    assert resp.status_code == 200, resp.text
    for part in resp.text.split("\n\n"):
        line = part.strip()
        if not line.startswith("data:"):
            continue
        event = json.loads(line.removeprefix("data:").strip())
        if event["type"] == "error":
            raise AssertionError(event["text"])
        if event["type"] == "result":
            return event["data"]
    raise AssertionError(f"missing result event: {resp.text}")


def _mock_review(monkeypatch):
    async def _review(_raw_text, preliminary, _cfg):
        return [
            {**item, "review": "confirmed", "corrected_name": None}
            for item in preliminary
        ]

    monkeypatch.setattr(ocr_router, "review_fund_matches", _review)


class TestOcrFundCode:
    def test_codes_found_and_verified(self, ocr_client, fake_search, monkeypatch):
        monkeypatch.setattr(ocr_router, "ocr_text", lambda path: "005827")

        async def extract(_raw_text, _cfg):
            return [{"code": "005827", "name": "", "amount": 1234.56}]

        monkeypatch.setattr(ocr_router, "extract_funds_from_text", extract)

        resp = ocr_client.post("/api/ocr/fund-code", files=_fund_upload())
        body = _sse_result(resp)
        assert body["matched_codes"] == ["005827"]
        assert body["matched_funds"] == [
            {"code": "005827", "name": "易方达蓝筹精选混合", "amount": 1234.56}
        ]
        assert body["name_matches"] == []
        assert body["image"] == "shot.png"

    def test_false_positive_code_dropped(self, ocr_client, fake_search, monkeypatch):
        monkeypatch.setattr(ocr_router, "ocr_text", lambda path: "持有金额100056元")

        async def extract(_raw_text, _cfg):
            return [{"code": "100056", "name": "", "amount": None}]

        monkeypatch.setattr(ocr_router, "extract_funds_from_text", extract)

        resp = ocr_client.post("/api/ocr/fund-code", files=_fund_upload())
        body = _sse_result(resp)
        assert body["matched_codes"] == []
        assert body["matched_funds"] == []

    def test_source_down_keeps_candidate(self, ocr_client, monkeypatch):
        monkeypatch.setattr(ocr_router, "ocr_text", lambda path: "005827")

        async def extract(_raw_text, _cfg):
            return [{"code": "005827", "name": "", "amount": None}]

        async def broken_search(q, limit=1):
            raise RuntimeError("source down")

        monkeypatch.setattr(ocr_router, "extract_funds_from_text", extract)
        monkeypatch.setattr(ocr_router, "search_fund_by_name", broken_search)

        resp = ocr_client.post("/api/ocr/fund-code", files=_fund_upload())
        body = _sse_result(resp)
        assert body["matched_codes"] == ["005827"]

    def test_name_fallback_when_no_codes(self, ocr_client, fake_search, monkeypatch):
        _mock_review(monkeypatch)
        monkeypatch.setattr(ocr_router, "ocr_text", lambda path: "招商中证白酒指数A")

        async def extract(_raw_text, _cfg):
            return [{"code": None, "name": "招商中证白酒指数A", "amount": None}]

        monkeypatch.setattr(ocr_router, "extract_funds_from_text", extract)

        resp = ocr_client.post("/api/ocr/fund-code", files=_fund_upload())
        body = _sse_result(resp)
        assert body["matched_codes"] == ["161725"]
        assert body["name_matches"][0]["code"] == "161725"
        assert body["name_matches"][0]["ocr_name"] == "招商中证白酒指数A"

    def test_name_fallback_merges_with_codes(
        self,
        ocr_client,
        fake_search,
        monkeypatch,
    ):
        _mock_review(monkeypatch)
        monkeypatch.setattr(ocr_router, "ocr_text", lambda path: "005827\n招商")

        async def extract(_raw_text, _cfg):
            return [
                {"code": "005827", "name": "", "amount": 2000.0},
                {"code": None, "name": "招商中证白酒指数A", "amount": None},
            ]

        monkeypatch.setattr(ocr_router, "extract_funds_from_text", extract)

        resp = ocr_client.post("/api/ocr/fund-code", files=_fund_upload())
        body = _sse_result(resp)
        assert body["matched_codes"] == ["005827", "161725"]
        assert [m["code"] for m in body["name_matches"]] == ["161725"]

    def test_unique_filenames_for_concurrent_uploads(self, ocr_client, monkeypatch):
        monkeypatch.setattr(ocr_router, "ocr_text", lambda path: "")

        async def extract(_raw_text, _cfg):
            return []

        monkeypatch.setattr(ocr_router, "extract_funds_from_text", extract)
        names = set()
        for _ in range(5):
            resp = ocr_client.post("/api/ocr/fund-code", files=_fund_upload())
            names.add(_sse_result(resp)["saved_at"])
        assert len(names) == 5


class TestOcrTransaction:
    def test_transaction_extracted(self, ocr_client, monkeypatch):
        tx = {
            "direction": "buy",
            "code": "005827",
            "trade_date": "2026-06-10",
            "nav": "2.5000",
            "shares": "400.00",
            "amount": "1000.00",
        }
        monkeypatch.setattr(
            ocr_router,
            "ocr_text",
            lambda path: "买入 005827 确认份额400.00",
        )

        async def extract_tx(_raw_text, _cfg):
            return tx

        monkeypatch.setattr(ocr_router, "extract_transaction_from_text", extract_tx)

        resp = ocr_client.post("/api/ocr/transaction", files=_png_upload())
        assert resp.status_code == 200
        body = resp.json()
        assert body["transaction"] == tx
        assert body["image"].startswith("ocr_tx_")
