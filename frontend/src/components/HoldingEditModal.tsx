...
export function HoldingEditModal({
  ...
  useEffect(() => {
    ...
    fetchFundDetail(code)
      .then((d) => {
        ...
        const rate =
          d.subscription_rate_discounted ?? d.subscription_rate ?? null
        setFeeRate(rate)
      })
      .catch(() => {
        /* silently ignore */
  ...
  const holdingSharesNum =
    holdingShares != null ? parseFloat(holdingShares) : NaN
  const applyPct = (pct: number | 'all') => {
    ...
    const newShares =
      pct === 'all'
        ? holdingShares
        : String(Math.floor(holdingSharesNum * pct * 100) / 100)
    setShares(newShares)
    const n = parseFloat(nav)
